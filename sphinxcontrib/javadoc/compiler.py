...
class JavadocRestCompiler(object):
    ...
    def __output_doc(self, documented):
        ...
        if doc.authors:
            output.add_line(
                ":author: %s" % (self.__html_to_rst(", ".join(doc.authors)),)
            )
        ...
        for exception in doc.throws:
            ...
            output.add_line(
                ":throws %s: %s" % (exception, self.__html_to_rst(description))
            )
    ...
    def compile_constructor(self, constructor):
        ...
        formatter.output_list(
            formatter.output_formal_param, constructor.parameters, signature, ", "
        )
        signature.append(")")
        ...
        if constructor.throws:
            ...
            formatter.output_list(
                formatter.output_exception, constructor.throws, signature, ", "
            )
    ...
    def compile_method(self, method):
        ...
        signature.append("(")
        formatter.output_list(
            formatter.output_formal_param, method.parameters, signature, ", "
        )
        signature.append(")")
        ...
        if method.throws:
            signature.append(" throws ")
            formatter.output_list(
                formatter.output_exception, method.throws, signature, ", "
            )
    ...
    def compile(self, ast):
        ...
        for path, node in ast.filter(javalang.tree.TypeDeclaration):
            ...
            classes = [
                n.name for n in path if isinstance(n, javalang.tree.TypeDeclaration)
            ]
        ...
        for package, name, declaration in type_declarations:
            ...
            document = self.compile_type_document(
                import_block, package, name, declaration
            )
